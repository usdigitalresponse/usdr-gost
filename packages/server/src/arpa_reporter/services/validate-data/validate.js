...
function dateIsOnOrBefore(key) {
    return (val, content) => new Date(val).getTime() <= new Date(content[key]).getTime();
}
...
function whenBlank(key, validator) {
    return (val, content, context) => !!content[key] || validator(val, content, context);
}
...
function whenUS(key, validator) {
    return (val, content, context) => !isUnitedStates(content[key])
      || validator(val, content, context);
}

function whenGreaterThanZero(key, validator) {
    return (val, content, context) => (content[key] > 0 ? validator(val, content, context) : true);
}
...
function validateRecords(tab, validations) {
    return function (groupedRecords, validateContext) {
        ...
        return _.flatMap(records, ({ content, sourceRow }) => validateFields(
            validations,
            content,
            tab,
            sourceRow,
...
// NOTE: This file was copied from src/server/services/validate-data/validate.js (git @ ada8bfdc98) in the arpa-reporter repo on 2022-09-23T20:05:47.735Z
